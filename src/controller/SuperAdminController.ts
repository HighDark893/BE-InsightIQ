...
router.post('/update', requireAuthentication, authorize(['SUPERADMIN']), async (req: Request, res: Response) => {
    try {
        const superAdminDto: SuperAdminDto = req.body;
...
router.get('/:id', requireAuthentication, authorize(['SUPERADMIN']), async(req: Request, res: Response) => {
    try {
